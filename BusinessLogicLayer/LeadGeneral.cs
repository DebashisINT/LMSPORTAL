      ...
      bool chkAllow, string password)
        {
            ...
            try
            {
                using (proc = new ProcedureExecute("LeadInsert"))
                {
                    ...
                    proc.AddVarcharPara("@lastModifyUser", 100, Convert.ToString(HttpContext.Current.Session["LMSuserid"]));
                    proc.AddVarcharPara("@UserContactID", 100, Convert.ToString(HttpContext.Current.Session["LMSusercontactID"]));
