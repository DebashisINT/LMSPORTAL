  ...
  public  class ReportLayout
    {
        public void insert_layout( byte[] data)
        {
            ...
            try
            {
                ...
                com.Parameters.AddWithValue("@CreatedBy", HttpContext.Current.Session["LMSuserid"]);
                com.ExecuteNonQuery();
                con.Close();
        ...
        public int copyLayout(int copyId, string fileName,string moduleKey)
        {
            ...
            {
                //SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                //SqlCommand com = new SqlCommand("insert into tbl_reportDesigner select LayoutData,'"+fileName+"','"+moduleKey+"',GETDATE()," +Convert.ToString( HttpContext.Current.Session["LMSuserid"]) + ",GETDATE() from tbl_reportDesigner where id=@id", con);
                //con.Open();
