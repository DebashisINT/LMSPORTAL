...
namespace BusinessLogicLayer
{
    public class Others
    {
        ...
        public string GetAvailableStockCheckForSCorBOorIST(DataTable duplicatedt,string BranchId)
        {
            ...
            if (duplicatedt != null && duplicatedt.Rows.Count > 0)
            {
                foreach (DataRow row in duplicatedt.Rows)
                {
                    ...
                    DataTable dtAvailableStockCheck = oDBEngine.GetDataTable("Select dbo.fn_CheckAvailableQuotation(" + BranchId + ",'" + Convert.ToString(HttpContext.Current.Session["LMSLastCompany"]) + "','" + Convert.ToString(HttpContext.Current.Session["LMSLastFinYear"]) + "'," + ProductId + ") as branchopenstock");
            ...
            if (duplicatedt != null && duplicatedt.Rows.Count > 0)
            {
                foreach (DataRow row in duplicatedt.Rows)
                {
                    ...
                    if (IsInventory != "N")
                    {
                        ...
                        BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                        //DataTable dtAvailableStockCheck = oDBEngine.GetDataTable("Select dbo.fn_CheckAvailableStockSCBOIST(" + BranchId + ",'" + Convert.ToString(HttpContext.Current.Session["LMSLastCompany"]) + "','" + Convert.ToString(HttpContext.Current.Session["LMSLastFinYear"]) + "'," + ProductId + "'," + Convert.ToDateTime(Date) + ") as branchopenstock");
                        DataTable dtAvailableStockCheck = oDBEngine.GetDataTable("Select dbo.fn_CheckAvailableStockSCBOIST(" + BranchId + ",'" + Convert.ToString(HttpContext.Current.Session["LMSLastCompany"]) + "','" + Convert.ToString(HttpContext.Current.Session["LMSLastFinYear"]) + "','" + ProductId + "','" + Convert.ToDateTime(Date).ToString("yyyy-MM-dd") + "') as branchopenstock");
        ...
        public DataTable GetQuotationOnSalesOrder(string customer, string OrderDate, string Status,Int32 BranchId)
        {
            ...
            proc.AddVarcharPara("@userbranchlist", 1000, Convert.ToString(HttpContext.Current.Session["LMSuserbranchHierarchy"]));
        ...
        public DataTable GetSalesInvoiceonSalesChallan(string customer, string ChallanDate, string Status, int BranchId)
        {
            ...
            proc.AddVarcharPara("@Status", 50, Status);
           // proc.AddIntegerPara("@Branch_Id", BranchId);
            proc.AddVarcharPara("@userbranchlist", 4000, Convert.ToString(HttpContext.Current.Session["LMSuserbranchHierarchy"]));
            dt = proc.GetTable();
            return dt;
        }
